...
conversation_cache = defaultdict(
    lambda: [{"role": "system", "content": PAPER_SYSTEM_PROMPT}]
)

url_cache = {}


class PaperHandler(PoeHandler):
    async def get_response(self, query: QueryRequest) -> AsyncIterable[ServerSentEvent]:
        ...
        if query.conversation_id not in url_cache:
            ...
            if content_url.endswith(".pdf"):
                success, paper_string = await parse_pdf_document_from_url(content_url)
            else:  # assume image
                success, paper_string = await parse_image_document_from_url(content_url)
            ...
            user_statement = PAPER_STARTING_PROMPT.format(paper_string)
